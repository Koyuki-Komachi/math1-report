...
def process_sdg_data():  
    ...
    try:  
        ...
        for goal in goals:  
            plt.plot(averages_df['year'], averages_df[goal], marker='o', label=goal.capitalize())  
        
        plt.title('Sub-Saharan Africaの各ゴールの年次平均値 (2000-2023)', fontsize=16)  
        plt.xlabel('年', fontsize=14)  
        plt.ylabel('平均スコア', fontsize=14)  
